...
def profile_most_probable_kmer_score(text: str, k: int,
...
def main(pwm_sequences, sequences, output_file):
    ...
    # Use the PWM to calcualte the most probable kmer score in each sequence
    predictions = []
    i = 1
    for seq in sequences:
        score = profile_most_probable_kmer_score(seq, K, profile)
        predictions.append((i, score))
        i += 1

    sorted_predictions = sorted(predictions, key=lambda x: x[1], reverse=True)

    # Get the top 6000 scoring ones
    with open(output_file, 'w') as file:
        for i in range(6000):
            file.write(f"seq{sorted_predictions[i][0]}\n")
    
    print("Success!")

if __name__ == "__main__":
    # Handle CLI
    parser = argparse.ArgumentParser(description="Project 3b Solution")
    parser.add_argument("-s", "--sequences", required=True, dest="sequences", help="Bound sequences file path to generate PWM")
    parser.add_argument("-i", "--input", required=True, dest="input", help="Tests file path to find highest probability ones using PWM")
